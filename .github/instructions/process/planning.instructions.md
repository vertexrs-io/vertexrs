...
| `ready` | Queue bot (`implementer-queue.yml`) | Slot granted **and** issue is trivial; triggers the Implementer automatically |
| `awaiting-design` | Queue bot (`implementer-queue.yml`) | Slot granted **and** issue is non-trivial; signals a human to run the Architect locally — nothing fires automatically |
| `design-approved` | **Human** (manual) | Architect session complete; Implementer may now build the design |

Notes:
- `awaiting-agent` and `design-approved` are the only transitions that require a human; agents must never apply these
- `ready` and `awaiting-design` are bot-applied so the Implementer workflow can distinguish bot-driven promotion from manual labelling. The two are mutually exclusive — the scheduler picks one based on the `trivial` label
...
If any criterion is not met, do not apply `trivial`. Non-trivial issues require a human to run a local Architect session (against the `awaiting-design` issue, on the `feat/<issue-number>-<slug>` branch) before setting `design-approved`; the Implementer must not start until then.
