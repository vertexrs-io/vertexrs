...
9. **Push** — immediately after CI passes, push all commits to the remote branch: `git push origin <branch-name>`. Do this before touching the PR. If the push fails, exit non-zero — do not proceed.
10. **Open PR** (or convert draft to ready) — title format `[Phase X.Y] Short description (#issue-number)`; body must include `Closes #<issue-number>` (see AGENTS.md → "Every PR closes an issue" — title-only references do not auto-close), then list acceptance criteria with checkmarks and link each to its corresponding test. For non-trivial issues you are converting the Architect's draft PR — verify the `Closes` line is present in the body and add it if missing.
