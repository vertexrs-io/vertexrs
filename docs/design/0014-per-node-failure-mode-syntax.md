...
## 2. User-Facing Macro Syntax

This section shows the exact syntax a user writes.  The Implementer must ensure every snippet below compiles and behaves as described.

### 2.1 Default (no annotation)

No sigil, no `pure` option.  Failure mode comes from the enclosing `settings { failure: … }` block, or from the pipeline default (soft).  Equivalent to `pure = true`.

```rust
pipeline! {
    source!(price: f64);
    node!(tax = price.row(|p| p * 0.2));
}
```

### 2.2 Soft-failure override — trailing `?`

The `?` sigil appears **after the closing `)` of the `node!` call**, not inside the closure.  A `?` inside a closure body is the standard Rust `Try` operator on an inner expression — the macro distinguishes by position.

```rust
pipeline! {
    source!(price: f64);
    node!(tax = price.row(|p| -> f64 {
        if p < 0.0 { panic!("negative price") }
        p * 0.2
    })?);
}
```
...
- `compute()` returns `Ok(())`
- The `tax` column is entirely NA (Arrow validity bitmap all zeros)
- `pipeline.drain_warnings()` returns at least one entry containing `"tax"`
- Downstream nodes that consume `tax` still execute, operating on the all-NA column

### 2.3 Hard-failure override — trailing `!`

The `!` sigil also trails the outer `)`.

```rust
pipeline! {
    source!(price: f64);
    node!(tax   = price.row(|p| -> f64 { panic!("hard fail") })!);
    node!(total = tax.row(|t| t + 1.0));       // never executes if tax panics
}
```

When the kernel panics:
- `compute()` returns `Err(PipelineError::KernelPanic(…))`
- Execution halts immediately; `total` is not recomputed
- The output frame retains its values from the last successful `compute()` call

### 2.4 Always-dirty node — `pure = false`

The `pure = false` option is a named key-value pair **after a comma** following the expression.  It may appear with or without a sigil.
...
Semantics now: compiles and runs normally; the resulting `Node<T>` has `pure == false`.  A future incremental executor will use this flag to mark the node always-dirty.

### 2.5 Combining a sigil with `pure = false`

The sigil comes **before** the comma:

```rust
pipeline! {
    source!(src: f64);
    node!(counter = src.row(|x| -> f64 { risky(x) })?, pure = false);
}
```

Both the soft-failure behaviour and the `pure = false` flag are applied.  The comma after the sigil is the option separator; there is no ambiguity because `?` always trails the expression and `,` always begins the option list.

### 2.6 Node-level sigil overrides pipeline-level setting

A per-node sigil always wins:

```rust
pipeline! {
    settings { failure: Hard }                    // pipeline default: hard
    source!(price: f64);
    node!(tax = price.row(|p| -> f64 { panic!() })?);  // `?` overrides → soft
    //   ↑ returns Ok(()), tax is all-NA
}

pipeline! {
    settings { failure: Soft }                    // pipeline default: soft
    source!(price: f64);
    node!(tax = price.row(|p| -> f64 { panic!() })!);  // `!` overrides → hard
    //   ↑ returns Err(KernelPanic(_))
}
```

### 2.7 Syntax summary table

| Written in `node!( … )` | Failure on kernel panic | `pure` flag |
|---|---|---|
| `name = expr` | pipeline default (soft if no `settings {}`) | `true` |
| `name = expr?` | **soft** — NA column, warning, `Ok(())` | `true` |
| `name = expr!` | **hard** — `Err(KernelPanic)`, halt | `true` |
| `name = expr, pure = false` | pipeline default | **false** |
| `name = expr?, pure = false` | **soft** | **false** |
| `name = expr!, pure = false` | **hard** | **false** |
